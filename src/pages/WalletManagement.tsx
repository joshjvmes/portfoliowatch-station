...
const WalletManagementContent = () => {
  ...
  useEffect(() => {
    ...
    (appKit as any).on('connect', handleConnect);
    (appKit as any).on('disconnect', handleDisconnect);

    return () => {
      (appKit as any).off('connect', handleConnect);
      (appKit as any).off('disconnect', handleDisconnect);
    };
  }, []);
